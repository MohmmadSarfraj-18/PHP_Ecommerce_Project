<?php 
    include('../controller/maleProductsController.php');
    require('header.php');
    $conn = new MaleProductsController();

    // Searching of Product
    if(isset($_REQUEST['search'])) {
        $record = $conn->search_mens_allProducts($_REQUEST);
    } else if($conn) {
        $record = $conn->fetch_allMaleProducts();
    }
    // Product Add TO Cart
    if(!empty($_REQUEST['add_to_cart'])) {
        if(isset($_SESSION['user_id'])) { 
            ...
            echo "<script>location.href='add_to_cart.php?pageName=maleProduct&productId=$productid&size=$size&quantity=$quantity&userId=$userId'</script>";
        } else {
...
?>
    ...
    <div class="container-fluid"> 
        <!-- male Products -->
        <div class="row">
            <h2 id="heading" class="pb-2 border-bottom mb-4 text-muted" id="male">Male Products</h2>
