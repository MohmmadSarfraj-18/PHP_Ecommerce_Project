<?php 
    ...
    require('header.php');
    $conn = new AllProductsController();

     // Searching of Product
     if(isset($_REQUEST['search'])) {
        // var_dump($_REQUEST);
        $record = $conn->fetch_search_allProducts($_REQUEST);

    } else if($conn) {
        $record = $conn->fetch_allProducts();
    }
    ...
    if(!empty($_REQUEST['add_to_cart'])) {
        if(isset($_SESSION['user_id'])) { 
            ...
            echo "<script>location.href='add_to_cart.php?pageName=allproduct&productId=$productid&size=$size&quantity=$quantity&userId=$userId'</script>";
        } 
        else {
...
?>
    ...
    <div class="container" style="margin-top: 5rem;"> 
        <!-- All Products -->
        <h2 id="heading" class="pb-2 border-bottom mb-4 text-muted">All Products</h2>
